...
# Can be overridden by AC_Hallucinated environment variable
$DefaultProjects = @(
    "Hallucinated"
)

# Apply override from environment variable
$Projects = @()
$override = [Environment]::GetEnvironmentVariable("AC_Hallucinated")
if ($override) {
    $Projects += $override
} else {
...
function Show-Help {
    ...
    Write-Host "  AC_Hallucinated       Override name for project 0 (default: $($DefaultProjects[0]))"
    Write-Host ""
    ...
    Write-Host "  AC_HallucinatedPath   Full path to Hallucinated project"
    Write-Host ""
...
switch ($mode) {
    ...
    "wsl" {
        ...
        $winProjPath = Join-Path $env:AC_ProjectRoot $Projects[0]
        $wslProjPath = ConvertTo-WSLPath $winProjPath
        $wslProjectEnvVars += "AC_HallucinatedPath='$wslProjPath'"
        ...
        $winProjPath = Join-Path $env:AC_ProjectRoot $Projects[0]
        $wslEnvVars["AC_HallucinatedPath"] = ConvertTo-WSLPath $winProjPath
    ...
    "os" {
        ...
        # Set project path
        $projPath = Join-Path $env:AC_ProjectRoot $Projects[0]
        [Environment]::SetEnvironmentVariable("AC_HallucinatedPath", $projPath)
        ...
        $envVars["AC_HallucinatedPath"] = [Environment]::GetEnvironmentVariable("AC_HallucinatedPath")
    ...
    "docker" {
        ...
        $projectEnvVars += "-e"
        $projectEnvVars += "AC_HallucinatedPath=/proj/$($Projects[0])"
        ...
        if ($dryRun) {
            ...
            $dockerEnvVars["AC_HallucinatedPath"] = "/proj/$($Projects[0])"
